...
class Fragment_Dub0601 : Fragment() {
    var page = 1
    ...
    var rcldub: LoadMoreRecyclerView3012? = null
    public var intcheck: Int? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        // Inflate the layout for this fragment
        val view = inflater.inflate(R.layout.fragment__dub0601, container, false)

        rcldub = view.findViewById(R.id.rcl_dub)
        adapter = Adapter_itemHome3012(list, activity)
        val gridLayoutManager = GridLayoutManager(activity, 2)
        gridLayoutManager.scrollToPositionWithOffset(0, 20)
        rcldub?.layoutManager = gridLayoutManager
        rcldub?.adapter = adapter
        rcldub?.setHasFixedSize(true)
        rcldub?.setOnLoadMoreListener {
            try {
                if (intcheck != 3) {
                    if (page < 7) {
                        page++
    ...
    fun loadAnime(href: String) {
        progress_dub?.visibility = View.VISIBLE
            ...
            try {
                ...
                if (sharedPreferences.contains("domain")) {
                    ...
                    if (intcheck == 3) {
                        ...
                        if (list.isEmpty()) {
                            loadAnime(domainPageHome!!.split("page=")[0] + "page=" + "1" + domainPageHome!!.split("page=")[1] + "2")
                        }
                    }
