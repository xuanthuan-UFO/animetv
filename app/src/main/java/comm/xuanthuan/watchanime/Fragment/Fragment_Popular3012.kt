...
import kotlinx.android.synthetic.main.fragment__popular3012.*
import org.jsoup.Jsoup

class Fragment_Popular3012 : Fragment() {

    var list: ArrayList<Object_Anime3012> = ArrayList()
    var adapter: Adapter_itemHome3012? = null
    var rcl_popular: LoadMoreRecyclerView3012? = null
    var i = 1
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment__popular3012, container, false)
        rcl_popular = view?.findViewById(R.id.rcl_popular)
        adapter = Adapter_itemHome3012(list, activity)
        rcl_popular?.adapter = adapter
        val gridLayoutManager = GridLayoutManager(activity, 2)
        gridLayoutManager.scrollToPositionWithOffset(0, 20)
        // rcl_newSeason.setLayoutManager(gridLayoutManager)
        rcl_popular?.layoutManager = gridLayoutManager
        rcl_popular?.setHasFixedSize(true)
        rcl_popular?.setOnLoadMoreListener(object : LoadMoreRecyclerView3012.OnLoadMoreListener {
            override fun onLoadMore() {
                if ((activity as Activity_Home3012).intcheck != 3) {
                    if (i < 5) {
                        i++
                        try {
                            loadAnime((activity as Activity_Home3012).domainPopular.toString() + "?page=" + i)
                        } catch (e: Exception) {
                        }
                    }
                }
    ...
    fun loadItem(responseString: String) {
        try {
            ...
            try {
                ...
                rcl_popular?.finishLoading()

            } catch (e: Exception) {
            }
            container_popular?.visibility = View.INVISIBLE
        } catch (e: Exception) {
            container_popular?.visibility = View.INVISIBLE
        }
    }
    ...
    override fun setUserVisibleHint(isVisibleToUser: Boolean) {
        ...
        if (isVisibleToUser) {
            try {
                if ((activity as Activity_Home3012).intcheck != 3) {
                    if (list.isEmpty()) {
                        loadAnime((activity as Activity_Home3012).domainPopular.toString())
                    }
                } else {
                    if (list.isEmpty()) {
